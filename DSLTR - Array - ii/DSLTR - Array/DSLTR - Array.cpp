    ...
    */
    ...
    for (i = 1; i < n; ++i) {
        if (ascending) {
            ...
            key = data[i-1].food; // 1 hou
            int j = i ; //0 qian

            // Move elements of arr[0..i-1] that are greater than key to one position ahead of their current position
            while (j >= 0 && data[j].food > key) { // qian > hou
                data[i - 1].food = data[i].food; // 1 = 0 //qian diu qu hou
                i = i - 1;//-1
            }

            data[i + 1].food = key; //hou diu qu qian
        }
    ...
    for (int i = 1; i < n - 1; i++) {
        ...
        while (j >= 0) {
            ...
            switch (column) {
            case 1:  // Sorting based on food name
                if ((ascending && data[j].food > key.food) || (!ascending && data[j].food < key.food)) {
                    data[j + 1] = data[j];
                    j--;
                }
                else {
                ...
                break;
            case 2:  // Sorting based on measure
                if ((ascending && data[j].measure > key.measure) || (!ascending && data[j].measure < key.measure)) {
                    data[j + 1] = data[j];
                    j--;
                }
                else {
